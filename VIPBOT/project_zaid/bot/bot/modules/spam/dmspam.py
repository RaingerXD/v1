...
from bot.modules.help import *
import os
import sys
import asyncio
from random import choice
from bot.modules.sudo import OWNER, SUDO_USERS
from pyrogram import Client, filters
from pyrogram.types import Message
from cache.data import *
from bot.mdoules.sudo import SUDO_USER
...
async def dmraid(xspam: Client, e: Message):
      """ Module: Dm Raid """
      bot = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
      if len(bot) == 2:
          ok = await xspam.get_users(bot[1])
          id = ok.id
          if int(id) in VERIFIED_USERS:
                text = f"Ayo, jangan ajari"
