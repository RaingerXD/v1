...
from bot.database import gbandb as bot
from bot.database import gmutedb as Gmute
...
async def gban_user(client: Client, message: Message):
    ...
    if (await bot.gban_info(user.id)):
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await bot.gban_info(user.id)):
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await bot.gban_info(user_id)):
        try:
