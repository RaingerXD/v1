...
from bot.database.pmpermitdb import get_approved_users, pm_guard
import bot.database.pmpermitdb as bot
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await bot.set_block_message(bot.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await bot.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await bot.get_pm_settings()
    await bot.allow_user(chat_id)
