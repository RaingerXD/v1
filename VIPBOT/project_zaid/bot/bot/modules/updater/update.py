...
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: bot-Userbot Deploy Update is in Progress...`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit(
            "`bot-Userbot Successfully Updated! Userbot can be used again.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
...
add_command_help(
    "update",
    [
        ["update", "To see a list of the latest updates from bot-Userbot."],
