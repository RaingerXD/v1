...
async def joinvc(client: Client, message: Message):
    ...
    try:
        ...
        return await ky.edit("`Timeout! Keluar dari obrolan suara.{chat_id}`")
    except Exception as e:
        return await ky.edit(f"ERROR: {e}")
    finally:
        await client.group_call.stop()
    await ky.edit(f"`Timeout! Keluar dari obrolan suara. {chat_id}`")
...
add_command_help(
    "vctools",
    [
        [f"joinvc", "Join ke obrolan suara."],
        [f"leavevc", "Keluar obrolan suara."],
        [f"startv", "Start voice chat group."],
        [f"stopv", "End voice chat group."],
    ],
)
